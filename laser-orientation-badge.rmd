...
-   Some variables are numerical, whereas some are non-numerical
    character
...
```{r}
d |> 
  select(student_id, total_points_possible, total_points_earned, course_id)
```
...
```{r}
d |>
  select(student_id, total_points_possible, FinalGradeCEMS) |>
  filter(FinalGradeCEMS > 60) |>
  arrange(total_points_possible)
```
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + subject + int + uv, data = d)
summary(m2)
```
